...
@Component({
  ...
  templateUrl: './issue-transfer.component.html',
  styleUrl: './issue-transfer.component.css'
})
export class IssueTransferComponent implements OnInit {
  ...
  addProspect(prospectForm : NgForm) {
    // this.prospectService.saveProspect$(this.prospectForm.value).subscribe(response => {
    //   this.dataSubject.next([response, ...this.dataSubject.value]);
    //   this.newProspect = response;
